...
def place_elves(input_data) -> set:
    elves = set()
...
def move_elves(elves, round):
    first_dir = round % len(directions)
    wants_to_move = {}
    ...
    for elf_pos, new_pos in wants_to_move.items():
        if new_pos in clean_moves.values():
            # inefficient... remove the move that makes elves land on the same spot
